...
namespace MSS.API.Common.Utility
{
    public class HttpClientHelper<T> : IHttpClientHelper<T>
    {
        ...
        {
            T result = default(T);
            var response = await Client.PostAsync(apiUrl, new StringContent(JsonConvert.SerializeObject(postObject), Encoding.UTF8, "application/json"), cancellationToken);
            if (response.IsSuccessStatusCode)
            {
                await response.Content.ReadAsStringAsync().ContinueWith((Task<string> x) =>
                {
                    result = JsonConvert.DeserializeObject<T>(JsonConvert.SerializeObject(x.Result));
                }, cancellationToken);
            }
            else
            {
        ...
        {
            var response = await Client.PutAsync(apiUrl, new StringContent(JsonConvert.SerializeObject(putObject), Encoding.UTF8, "application/json"), cancellationToken).ConfigureAwait(false);
            if (!response.IsSuccessStatusCode)
            {
                var content = await response.Content.ReadAsStringAsync();
                response.Content?.Dispose();
